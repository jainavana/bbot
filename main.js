...
create({
  ...
  client.onMessage(async message => {
    const { from, body, sender, isGroupMsg, chatId, mentionedJidList } = message;
    ...
    switch (cmd) {
      ...
      case 'member': {
        if (!isAdmin(senderId, groupId)) return reply('❌ Permission denied.');
        const location = args[0];
        const mentionedJids = message.mentionedJidList || [];
        if (!mentionedJids.length || !location) return reply('⚠️ Usage: *bb member <location> @user*');
        addMember(groupId, mentionedJids[0], location);
    ...
    case 'members': {
        ...
        const membersMap = groupMembers.get(groupId);
        if (!membersMap) return reply(`❌ No members found for *${location}*.`);

        const members = [...membersMap.entries()]
            .filter(([_, locs]) => locs.has(location.toLowerCase()))
            .map(([jid], i) => {
            const contact = client.getContact(jid);
            return `${i + 1}. ${contact.pushname || contact.verifiedName || jid.split('@')[0]}`;
            });
